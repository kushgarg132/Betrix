...
public class GameValidatorService {
    ...
    /** The caller's own seat at this table, looked up by username. There is no client-supplied
     * playerId to check any more -- the caller can only ever act as themselves. */
    public Player requireOwnPlayer(Game game, String username) {
        Player player = username != null ? game.getPlayerByUsername(username) : null;
        if (player == null) {
            throw new AccessDeniedException("Not seated at this table");
        }
        return player;
